...
fun FrequencySelector(
    ...
    Column(modifier = modifier.padding(horizontal = 16.dp, vertical = 8.dp)) {
        Row(
            ...
            Text(stringResource(id = R.string.frequency_selector_title), style = MaterialTheme.typography.titleLarge, fontWeight = FontWeight.Medium)
        ...
        when (selectedFrequency) {
            FrequencyType.ONCE_A_DAY -> {
                SectionTitle(stringResource(id = R.string.freq_daily_reminder_time_days))
                OutlinedButton(
                    onClick = {
                    ...
                    Text(onceADayTime?.format(uiTimeFormatter) ?: stringResource(id = R.string.freq_select_reminder_time_button))
                }
            ...
            FrequencyType.INTERVAL -> {
                SectionTitle(stringResource(id = R.string.freq_repetition_interval_title))
                IntervalDurationSelector(intervalHours, intervalMinutes, onIntervalHoursChanged, onIntervalMinutesChanged)

                Spacer(Modifier.height(16.dp))
                SectionTitle(stringResource(id = R.string.freq_daily_active_range_label))
                Row(Modifier.fillMaxWidth().padding(bottom = 8.dp), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    TimeRangeButton(
                        label = stringResource(id = R.string.freq_start_time_button_label),
                        time = intervalStartTime,
                        placeholder = stringResource(id = R.string.freq_set_start_placeholder),
                        onClick = {
                    ...
                    TimeRangeButton(
                        label = stringResource(id = R.string.freq_end_time_button_label),
                        time = intervalEndTime,
                        placeholder = stringResource(id = R.string.freq_set_end_placeholder),
                        onClick = {
    ...
    if (showTimePickerFor != null) {
        TimePickerDialog(
            title = when (showTimePickerFor) { // Title for the dialog
                TimePickerTarget.ONCE_A_DAY -> stringResource(id = R.string.freq_select_reminder_time_button)
                TimePickerTarget.CUSTOM_ALARM -> stringResource(id = R.string.freq_add_alarm_time_dialog_title)
                TimePickerTarget.INTERVAL_START -> stringResource(id = R.string.freq_start_time_button_label)
                TimePickerTarget.INTERVAL_END -> stringResource(id = R.string.freq_end_time_button_label)
                ...
                TextButton(onClick = {
                    ...
                    when (showTimePickerFor) {
                        ...
                        TimePickerTarget.INTERVAL_END -> {
                            if (intervalStartTime == null || selectedLocalTime.isAfter(
                                    intervalStartTime
                                )
                            ) {
...
fun DaySelector(
    ...
    val daysOfWeekLabels = remember {
        listOf(
            R.string.day_mon_initial, // Assuming new keys for initials
            R.string.day_tue_initial,
            R.string.day_wed_initial,
            R.string.day_thu_initial,
            R.string.day_fri_initial,
            R.string.day_sat_initial,
            R.string.day_sun_initial
        )
    }

    Column(modifier = Modifier.fillMaxWidth()) {
        Text(stringResource(id = R.string.freq_repeat_on_days_label), style = MaterialTheme.typography.titleSmall, modifier = Modifier.padding(bottom = 12.dp))
        Row(
...
fun CustomAlarmsSelector(
    ...
    Column(modifier = Modifier.fillMaxWidth()) {
        SectionTitle(stringResource(id = R.string.freq_custom_alarm_times_title))
        Button(
            onClick = onShowTimePicker,
            modifier = Modifier.fillMaxWidth(),
        ) {
            Icon(Icons.Default.Add, stringResource(id = R.string.freq_add_new_alarm_button) , Modifier.size(ButtonDefaults.IconSize))
            Spacer(Modifier.size(ButtonDefaults.IconSpacing))
            Text(stringResource(id = R.string.freq_add_alarm_time_dialog_title)) // Or freq_add_new_alarm_button depending on context
        }

        if (selectedTimes.isNotEmpty()) {
            Spacer(Modifier.height(16.dp))
            Text(stringResource(id = R.string.freq_scheduled_times_label), style = MaterialTheme.typography.titleSmall, modifier = Modifier.padding(bottom = 8.dp))
            FlowRow(
                modifier = Modifier.fillMaxWidth(),
            ) {
                    ...
                    InputChip(
                        ...
                        trailingIcon = {
                            IconButton(
                                ...
                                Icon(Icons.Filled.Close, contentDescription = stringResource(id = R.string.freq_delete_time_acc, time.format(timeFormatter)))
                            }
                        },
                    )
                }
...
// Ensure com.d4viddf.medicationreminder.ui_new.common.components.TimePickerDialog is imported at the top of the file.
